...
function [r,t]=tmmcalc(q,dz,detuning)

N=length(q);
Nlambda=length(detuning);
...
for j=1:N						% sections
   ...
   for m=1:Nlambda				% freq.
      p=sqrt(qabs^2-detuning(m)^2);
      T=[cosh(p*dz)+i*detuning(m)/p*sinh(p*dz) q(j)/p*sinh(p*dz) ;conj(q(j))/p*sinh(p*dz) cosh(p*dz)-i*detuning(m)/p*sinh(p*dz)];
