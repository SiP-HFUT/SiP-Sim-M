% ---- Calculate the drop- and through-port responses of a mrr with two
% GA-CDCs as the light couplers; by Cheng Rui, 2022; 
...
N=260; % define total period number of the Bragg grating
...
l_total = 175e-6; % l_total: total length of the ring (including the grating);
l_ng = l_total - 2*l_g; % l_ng: length of the non-grating part of the ring
if l_ng <=0
    fprintf('Error! the ring is not long enough to accommodate the gratings; \n');
return;
end
...
drop = simplify(S.a3/a0);
drop_char = char(drop);
thr_char = char(thr);

% add a phase shift (ps) to the ring to fine tune the resonate wavelength to match the Bragg resonance of the grating
for ps = [0.25]* pi
